...
public class ShoppingCartTest {
    ...
    void runAllTests() {
        ...
        pass = tester.addItem_validItemWithPositiveQuantity_itemAdded() && pass;
        ...
        // PARTICIPANTS: uncomment the following lines, replacing [your test case method name] with calls to
        //               your actual test methods
        pass = tester.updateQuantity_updateExistingItemWithPositiveQuantity_quantityIsUpdated() && pass;
        // pass = tester.[your test case 1 method name]() && pass;
        // pass = tester.[your test case 2 method name]() && pass;
    ...
    private boolean addItem_validItemWithPositiveQuantity_itemAdded() {
        // GIVEN - an empty ShoppingCart
        ShoppingCart cart = new ShoppingCart();

        // WHEN - add an item with valid itemName and positive quantity
        boolean result = cart.addItem("validItem", 1);

        // THEN - add to cart should have succeeded, and ShoppingCart should contain item
        if (!result) {
            System.out.println("  FAIL: Adding item '' should have succeeded");
            return false;
        }

        if (!cartContainsItemAndQuantity(cart, "validItem", 1)) {
            System.out.println(
                    String.format("  FAIL: Adding item 'validItem' to ShoppingCart should contain item, but does not.")
            );
            return false;
        }

        System.out.println("  PASS: Adding 'validItem' with quantity 1 to ShoppingCart succeeded.");
        return true;
    }
    ...
    private boolean updateQuantity_updateExistingItemWithPositiveQuantity_quantityIsUpdated() {
        // GIVEN - A ShoppingCart with one type of item with a quantity of one.
        String itemName = "Existing Item";
        ShoppingCart cart = new ShoppingCart();
        cart.addItem(itemName, 1);

        // WHEN - Update the quantity by calling `updateQuantity()` for the item with a quantity of 3.
        boolean result = cart.updateQuantity(itemName, 3);

        // THEN - update is allowed and `getItems()` response includes the original item with updated quantity.
        if (!result) {
            System.out.println(String.format("  FAIL: Failed to update quantity on item \"%s\"", itemName));
            return false;
        }
        if (!cartContainsItemAndQuantity(cart, itemName, 3)) {
            System.out.println(
                    String.format("  FAIL: After setting quantity to 3, expected to find item \"%s\" in ShoppingCart",
                            itemName)
            );
            return false;
        }
        ...
        System.out.println(
                String.format("  PASS: After setting quantity to 3, ShoppingCart contains item \"%s\" with correct quantity.",
                        itemName)
        );
        return true;
    }
